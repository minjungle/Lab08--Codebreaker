...
//should come back here and change the url to localhost.
...
function createGameBoard() {
  ...
  for (var i = 1; i <= CODE_LENGTH; i++) {
    ...
    //add a dummy image
    $(newImg).attr("src", `./images/ball_${i}.png`);
  ...
  for (var i = NUM_ATTEMPTS; i > 0; i--) {
    ...
    $(resultsSpan).attr("id", `attempt${i}result`);
    $(newDiv).append(resultsSpan);
...
function process_attempt() {
  ...
  if (!attempt_code.includes(0)) {
    ...
    $(this)
      .parent()
      .css({ top: btn_initial_top - current_attempt_id * step + "px" });
...
function response(data, status) {
  ...
  if (response["action"] == "generateCode") {
    ...
    if (current_attempt_id < NUM_ATTEMPTS && !win) {
      ...
      win
        ? alert("GG! You win. Click OK to see result, and play again.") // won!!!
